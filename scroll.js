...
/*------------------------*/
InfiniteScroller = function(scroller, source) {
...
ContentSource.prototype = {
    ...
    return new Promise(function(resolve, reject) {
      setTimeout(function() {
        ...
        for (var i = 0; i < Math.abs(count); i++) {
          items[i] = {
            value: this.index,
            height: Math.round(Math.random() * (60 - 15) + 15) 
          };
  ...
  render: function(item, div) {
    ...
    div.querySelector("p").innerHTML = "OK " + item.value + " " + item.height + "px";
    div.querySelector("p").style.height = item.height + "px";
    if(item.self) {
